...
class TransReID(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, stride_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        self.blocks = nn.ModuleList([
            ...
            for i in range(depth)])#####depth是12，一共12个block
    ...
    def forward_features(self, x1,x2, camera_id, view_id):####################主要结构
        B = x1.shape[0]##一批图片个数
        x1 = self.patch_embed(x1)##把图片转换成batch，两种方法。
        x2 = self.patch_embed(x2)
        ## 一种做法是直接把Image转化成Patch，另一种做法是把Backbone输出的特征转化成Patch
        x = []
        for i in range(0,len(x1)):
            xi = torch.einsum("ab,ab->ab", [x1[i], x2[i]])
            x.append(xi)
        x = torch.stack(x)

        cls_tokens = self.cls_token.expand(B, -1, -1)  # 加入cls token
        ...
        if self.cam_num > 0 and self.view_num > 0:
            ...
            x = x + self.pos_embed  # 引入 Position Embedding后，还是197*768

        x = self.pos_drop(x)###dropout，之后维度应该没变
        ...
        if self.local_feature:
            ...
            x = self.norm(x)###变成197*768

            return x[:, 0]####变成1*768

    def forward(self, x1,x2, cam_label=None, view_label=None):
        x = self.forward_features(x1,x2, cam_label, view_label)
        return x
